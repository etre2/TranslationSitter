...
class Etre_TranslationSitter_Adminhtml_Translation_LogController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function importAction()
    {
        ...
        if ($isPosting) {
            if($this->isAdminStore()){
                ...
                $this->addError((new Exception("Attempting to upload translations to Admin store ID is not allowd")), "Please choose a store");
            }
        ...
        if ($this->validFileName($fileFieldName = 'translationsitter_source')) {
            try {
                ...
                if ($uploader->save($path, $tmpFileName)) {
                    ...
                    $file                  = $this->loadFile($fileFieldName, $filePath);

                    $this->newTranslations = $this->excelToArrayWithHeaders($file);
                    ...
                    if(!$this->hasError) $this->_getSession()->addSuccess(
                        Mage::helper('etre_translationsitter')->__("Translations imported successfully")
                    );
                }
            }  catch (Exception $exception) {
                $this->addError($exception, "There was a problem processing your new translations. Please check the Magento log for more info.");
            }
        }
     ...
     */
    protected function getUploader($fileFieldName): Varien_File_Uploader
    {
     ...
     */
    protected function updateItemsByKey($translationCollection)
    {
        ...
        try {
            ...
            $this->addError($exception,"There was a problem inserting the new records. Please check Magento logs for more info.");
        }
    }
    ...
    protected function insertNew()
    {
        ...
        foreach ($updateable as $updatedTranslation)
        {
            ...
            if(in_array($sourceString,$updated)) continue;
            ...
            $translation->setTranslationsitterSource($updatedTranslation['translation_source']?$updatedTranslation['translation_source']:'Import' );
     ...
     * return void;
     */
    protected function updateByKey($keyIds)
    {
     ...
     * @return bool
     */
    protected function hasRequiredFields($updatedTranslation): bool
    {
     ...
     * @return string
     */
    protected function getLikeModuleString($sourceText): string
    {
        return sprintf(
            '%s%s',
    ...
    protected function deleteOld($newTranslations, $keyIds)
    {
        foreach ($newTranslations as $updatedTranslation) {
            ...
            if (!$this->hasRequiredFields($updatedTranslation)) continue;
            ...
            if ($oldTranslations->getSize() > 0) {
                try {
                    foreach($oldTranslations as $oldTranslation) {
                        if(!($updatedTranslation->getSource()===$this->stringWithoutModule($oldTranslation))) continue;
     ...
     * @param $errorMessage
     */
    protected function addError($exception, $errorMessage)
    {
        $this->hasError = true;
     ...
     * @return mixed
     */
    protected function stringWithoutModule($oldTranslation)
    {
     ...
     */
    protected function getColumnStyle($columnName, $headingsArray, $highestRow, $worksheet)
    {
     ...
     */
    protected function getColumnLetter($columnName, $headingsArray)
    {
     ...
     * @return int
     */
    protected function getColumnIndex($sourceColumnId): int
    {
