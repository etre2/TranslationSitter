...
class Etre_TranslationSitter_Block_Log_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if (!Mage::app()->isSingleStoreMode()) {
            /** @var Mage_Adminhtml_Model_System_Store $systemStoreSingleton */
            $systemStoreSingleton = Mage::getSingleton('adminhtml/system_store');
            $fieldset->addField('store_id', 'select', [
                'name' => 'store_id',
                'title' => $this->_getHelper()->__('Apply only to this store view'),
                'label' => $this->_getHelper()->__('Apply only to this store view'),
                'class' => 'required-entry',
                'required' => true,
                'values' => $systemStoreSingleton
        ...
        if ($model) {
            ...
            $translationSource = $model->getTranslationsitterSource() == "" ? Mage::helper('etre_translationsitter')->__("TranslationSitter: %s", $userInfo) : $model->getTranslationsitterSource();
